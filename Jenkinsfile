...
// which are part of the Decon repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="decon-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /decon-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "deconpay-decon/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from deconpay-decon/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'deconpay-decon/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from deconpay-decon/develop"
            }
          }

          if (hasCache) {
            sh "cd /decon-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /decon-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /decon-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /decon-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /decon-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /decon-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /decon-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

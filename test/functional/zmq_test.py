...
from test_framework.mininode import deconhash
...
def deconhash_helper(b):
    return encode(deconhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that decon has been built with ZMQ enabled
        ...
        if not config["components"].getboolean("ENABLE_ZMQ"):
            raise SkipTest("decond has not been built with zmq enabled.")
    ...
    def _zmq_test(self):
        ...
        assert_equal(genhashes[0], deconhash_helper(body[:80]))
        ...
        for x in range(n * 4):
            ...
            if topic == b"hashblock":
                ...
                blockcount += 1
            if topic == b"rawblock":
                zmqRawHashed.append(deconhash_helper(body[:80]))
                msgSequence = struct.unpack('<I', msg[-1])[-1]

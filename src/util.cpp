...
const char * const BITCOIN_CONF_FILENAME = "decon.conf";
const char * const BITCOIN_PID_FILENAME = "decond.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DeconCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DeconCore
    // Mac: ~/Library/Application Support/DeconCore
    // Unix: ~/.deconcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DeconCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DeconCore";
#else
    // Unix
    return pathRet / ".deconcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty decon.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override decon.conf

...
BOOST_AUTO_TEST_CASE(netbase_splithost)
{
    ...
    BOOST_CHECK(TestSplitHost("127.0.0.15040, "127.0.0.1", 5040));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]", "127.0.0.1", -1));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]5040, "127.0.0.1", 5040));
    BOOST_CHECK(TestSplitHost("::ffff:127.0.0.1", "::ffff:127.0.0.1", -1));
    BOOST_CHECK(TestSplitHost("[::ffff:127.0.0.1]5040, "::ffff:127.0.0.1", 5040));
    BOOST_CHECK(TestSplitHost("[::]5040, "::", 5040));
    BOOST_CHECK(TestSplitHost(":5040, "::5040", -1));
    BOOST_CHECK(TestSplitHost("5040, "", 5040));
    BOOST_CHECK(TestSplitHost("[]5040, "", 5040));
...
BOOST_AUTO_TEST_CASE(netbase_lookupnumeric)
{
    ...
    BOOST_CHECK(TestParse("127.0.0.15040, "127.0.0.1:5040"));
    BOOST_CHECK(TestParse("::ffff:127.0.0.1", "127.0.0.1:65535"));
    BOOST_CHECK(TestParse("::", "[::]:65535"));
    BOOST_CHECK(TestParse("[::]5040, "[::]:5040"));
    BOOST_CHECK(TestParse("[127.0.0.1]", "127.0.0.1:65535"));
...
BOOST_AUTO_TEST_CASE(netbase_getgroup)
{
    ...
    BOOST_CHECK(ResolveIP("2002:102:30450405040:5040:5040:5040").GetGroup() == std::vector<unsigned char>({(unsigned char)NET_IPV4, 1, 2})); // RFC3964
    BOOST_CHECK(ResolveIP("2001:050405040:5040:5040:FEFD:FCFB").GetGroup() == std::vector<unsigned char>({(unsigned char)NET_IPV4, 1, 2})); // RFC4380
    ...
    BOOST_CHECK(ResolveIP("2001:470:abcd50405040:5040:5040:5040").GetGroup() == std::vector<unsigned char>({(unsigned char)NET_IPV6, 32, 1, 4, 112, 175})); //he.net
    BOOST_CHECK(ResolveIP("2001:200150405040:5040:5040:5040:5040").GetGroup() == std::vector<unsigned char>({(unsigned char)NET_IPV6, 32, 1, 32, 1})); //IPv6

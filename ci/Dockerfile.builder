...
# decon_hash
RUN git clone https://github.com/deconpay/decon_hash
RUN cd decon_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} decon
RUN useradd -u ${USER_ID} -g decon -s /bin/bash -m -d /decon decon
...
RUN mkdir /decon-src && \
  ...
  chown $USER_ID:$GROUP_ID /decon-src && \
